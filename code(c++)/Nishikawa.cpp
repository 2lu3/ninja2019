...
// 最初の一回だけ実行されます
void Game1_Nishikawa::setup(void)
{
    // コマンドプロンプトの文字をすべて消します
    system("cls");
    UserGame1::setup();
    // ドットの情報を代入します
    InputDotInformation();
...
void Game1_Nishikawa::loop()
{
    ...
    if (PositionX != 0 || PositionY != 0)
    {
        ...
        // 今のドット付近のドットのarrived_numをすべて++する。(Position Lost Area対策)
    ...
    {
        // Position Lost Area内なら
        PositionX = -1;
        // 今のドット付近のドットのarrived_timesをすべて+=3する
    ...
    if (SuperObj_Num != 0)
    {
        ...
        {
            // まだ、log_superobjに何も値が登録されていない場合
        ...
        {
            // すでにlog_superobjに何らかの値が登録されている場合は、その値とSuperObj_Xの値を比べる
    ...
    else if (IsOnSuperObj() && SuperObj_Num == 0 && log_superobj_num > 0)
    {
        ...
        // 保存されているSuperObjの値の中から、今の座標と最も近いSuperObjを見つける
        int min = 10000;
        int num = 0;
        ...
        // 見つかったSuperObjとの直線距離の二乗が1200未満なら、そのSuperObjをとったものとする
        if (min < 1200)
        {
            // SuperObjは、配列として複数保存可能にしている。例えば、5つのSuperObjの値を保存していたとすると、
            // 0 1 2 3 4 5 <- SuperObjの座標が6つある
            // このうち、今回見つけたSuperObjが2だとする
            // その場合、4から順番に、3 -> 2と代入していき、結果的に2を消すことを目的とする
            // 具体的には
            // 0 1 3 3 4 5 : 3 -> 2に代入
            // 0 1 3 4 4 5 : 4 -> 3に代入
            // 0 1 3 4 5 5 : 5 -> 4に代入
            // 0 1 3 4 5 :  log_superobj_num--することで、記録されたsuperobjの数を減らす
        ...
        {
            // もし、直線距離の二乗が1200以上なら、相手のSuperObjなので、直進する
            motor(3, 3);
        }
    }
    else if (Duration > 0)
    ...
    {
        // GoInDotsは、GoInDots(x, y, x_wide, y_wide, color)であり
        // それぞれ、範囲を指定するときの、中心のx座標、y座標、xの幅(一辺の半分), yの幅(一辺の半分)、行きたい場所の床の色である
        // つまり、下の例だと、マップすべての中で、行きたい場所の色(収集箱)の座標を自動で探して、その場所へ行くとなっている
        searching_object = -1;
        GoInDots(180, 135, 180, 135, POINT_DEPOSIT);
    }
    // 終了時間が近づくと、自動でDepositAreaへ向かう
    else if (Time > 450 && (LoadedObjects > 2 || loaded_objects[SUPER_LOADED_ID] > 0))
    {
        LoadedObjects = 6;
    }
    else if (log_superobj_num > 0)
    {
        ...
        {
            // この場所(コードが実行される場合)は、すでにsuperobjに近い
            if (prev_repeated_num + 40 < getRepeatedNum())
            {
                // 前回実行されたときの、repeated numと40違うとき(つまり、40 x 60(=2400ms)以上superobjの近くから離れている場合)
                // ペナルティで遠ざけられたと判断して、same_time(しばらく探して見つからなかったら諦めるための変数)を初期化する(つまり、もう一度最初から探す)
                same_time = 0;
            }
            // しばらく探して見つからなかったら、どこかへ行く
            if (same_time > 10)
            {
                log_superobj_num = 0;
                printf("%d There is no superobj\n", getRepeatedNum());
            }
            // より精密にsuperobjの場所へ向かう
...
long Game1_Nishikawa::WhereIsMotor(void)
{
...
long Game1_Nishikawa::WhereIsColorSensor(void)
{
...
int Game1_Nishikawa::CheckNowDot(void)
{
...
int Game1_Nishikawa::IsNearYellow(int num, int x, int y)
{
...
int Game1_Nishikawa::GoToPosition(int x, int y, int wide_decide_x, int wide_decide_y, int wide_judge_arrived)
{
...
void Game1_Nishikawa::InputDotInformation(void)
{
...
int Game1_Nishikawa::GoToDot(int x, int y)
{
    // printf("%d %d\n", x * kSize, y * kSize);
...
int Game1_Nishikawa::GoToDots(int x, int y, int wide_decide_x, int wide_decide_y)
{
...
int Game1_Nishikawa::GoInDots(int x, int y, int wide_decide_x, int wide_decide_y, int color)
{
    //fprintf(logfile, " %d Start GoToDots(%d, %d, %d, %d)\n", getRepeatedNum(), x, y, wide_decide_x, wide_decide_y);
    // printf("GoToDots(): %d %d %d %d\n", x, y, wide_decide_x, wide_decide_y);
    static int prev_x = -1;
...
int Game1_Nishikawa::HowManyCurved(int id)
{
    /*
...
void Game1_Nishikawa::GoToAngle(int angle, int distance)
{
...
void Game1_Nishikawa::AutoStrategy(void)
{
    /*
	30 x 30のエリアを順番に行く
	*/
