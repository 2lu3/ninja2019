    ...
    // 壁の位置を計算
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            else if (x[0] < x[1])
            {
                ...
                for (long xi = x[0]; xi < x[1]; ++xi)
                {
                    long yj = x[0] + tilt *;
                    while (0 <= yj && yj < kDotHeight && ((y[0] <= yj && yj < y[1]) || (y[1] <= yj && yj < y[0])))
                    {
                    }
                }
                else
                ...
                for (long xi = x[0]; (x[0] < x[1] ? xi <= x[1] : xi >= x[0]); (x[0] < x[1] ? ++xi : --xi))
                {
                    for (long yj = y[0]; (y[0] < y[1] ? yj <= y[1] : yj >= y[1]); (y[0] < y[1] ? ++yj : --yj))
                    {
                    }
                }
